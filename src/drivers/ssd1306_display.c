...
LOG_MODULE_REGISTER(display_ssd1306, LOG_LEVEL_INF);
...
{
    int ret = ssd1306_display_write_cmd((uint8_t)(0xB0U | (page & 0x0FU)));
    if (ret < 0) {
        return ret;
    }

    ret = ssd1306_display_write_cmd((uint8_t)(0x00U | (col & 0x0FU)));
    if (ret < 0) {
        return ret;
    }

    return ssd1306_display_write_cmd((uint8_t)(0x10U | ((col >> 4) & 0x0FU)));
}
...
int ssd1306_display_init(void)
{
    ...
    /* SSD1306-oriented init sequence for 128x64, internal charge pump. */
    ...
    for (size_t i = 0; i < sizeof(init_seq); i++) {
        ret = ssd1306_display_write_cmd(init_seq[i]);
        if (ret < 0) {
...
int ssd1306_display_clear(void)
{
    ...
    for (uint8_t page = 0; page < DISP_PAGES; page++) {
        int ret = ssd1306_display_set_page_col(page, 0);
        if (ret < 0) {
            return ret;
        }

        ret = ssd1306_display_write_data(line, sizeof(line));
        if (ret < 0) {
            return ret;
        }
    }
